...
export const generateCarouselContent = async (entry) => {
  let {
    body: { topic, content },
    user: { _id, currentPageId, chatGPTVersion },
  } = entry;

  let prompt = `Act as an expert copywriter. Now, your task is to regenerate new content for carousel existing content is ${content}. Content length must be same as original content.
