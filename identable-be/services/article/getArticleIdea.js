...
export const getArticleIdea = async (entry) => {
  let {
    body: { goal },
  ...
  let prompt = `Act as an SEO expert. Generate five article idea using these goal [${goal}] as the Point of View being [${pointOfView}], The target audience is [${targetAudience}], and the post is intended to [${objective}]. The call to action is to [${callOfAction}]. Your response should not repeat or restate the original prompt or its parameters.
