...
export const generateInspireMePostPrompt = async (entry) => {
  try {
    let {
      settingData,
      pageData,
...
-Encourage your audience to take a specific action, such as visiting your ${website}, signing up for a demo, or contacting your sales team.`;
      ...
      if (inspireMeType == INSPIREME_TYPE.Industry_Trend) {
        prompt = `Generate a brief insight about a current trend in [${designation}], including a recent statistic. The insight should be forward-looking and thought-provoking. `;
      } else if (inspireMeType == INSPIREME_TYPE.ThoughtLeadership) {
        prompt = `Create a thought-provoking statement about the future of [${designation}] that challenges conventional wisdom. The statement should be bold but not controversial`;
      } else if (inspireMeType == INSPIREME_TYPE.ProductivityHacks) {
        prompt = `Generate a unique productivity hack for professionals in [${designation}] that can save significant time or improve efficiency. The hack should be specific, actionable, and not widely known.`;
      } else if (inspireMeType == INSPIREME_TYPE.ProfessionalTips) {
        prompt = `Provide one actionable professional tip for [${designation}] professionals that can significantly impact their career growth or skill set. Use clear, directive language.`;
      } else if (inspireMeType == INSPIREME_TYPE.IndustryPrediction) {
        prompt = `Create a short-term (6-12 month) prediction for [${designation}] based on current trends or recent developments. The prediction should be thought-provoking and encourage discussion.`;
      }

      prompt = `${prompt}
      Your Task is to generate a LinkedIn post using these topic [${topic}] as the Point of View being [${pointOfView}], The target audience is [${targetAudience}], and the post is intended to [${objective}]. The call to action is to [${callOfAction}]. Posts should be written in [${language}]. The tone of your writing should be [${aiTone}], and the level of formality should be [${formality}] and provide an authentic and personal touch.
