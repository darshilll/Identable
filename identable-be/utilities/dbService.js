...
const deleteManyRecords = async (modelName, query, options = {}) => {
  try {
    return await collections[modelName].deleteMany(query, options);
  } catch (error) {
    console.error(
      "<<<<<<<<<<<<deleteManyRecords>>>>>>>>>>>>>",
      modelName,
...
const deleteOneRecords = async (modelName, query, options = {}) => {
  try {
    return await collections[modelName].deleteOne(query, options);
  } catch (error) {
    console.error(
      "<<<<<<<<<<<<updateOneRecords>>>>>>>>>>>>>",
      modelName,
